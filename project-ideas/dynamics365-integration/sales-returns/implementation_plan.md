...
> [!NOTE]
> **Field name changes between tables, corrected during implementation.** The value survives header → invoice as expected, but the *field name* does not — `SalesTable.PurchOrderFormNum` becomes `CustInvoiceJour.PurchaseOrder` after posting (displayName "Requisition", confirmed via Microsoft's CDM schema for `Transaction/CustInvoiceJour`). Initial X++ implementation assumed the field kept the same name (`PurchOrderFormNum`) on both tables and only discovered the mismatch via a compiler error ("Table 'CustInvoiceJour' does not contain a field named 'PurchOrderFormNum'") — the earlier `SysTableBrowser` verification (Section 6.4.4) had only confirmed the *value* survives, not the exact underlying field name, since that tool displays the field's label ("Purchase order"), not its AOT name. All X++ joins in Section 6.4.2 use `CustInvoiceJour.PurchaseOrder`; only the OMS-side sync code (setting `CustomerRequisitionNumber`/`PurchOrderFormNum` on `SalesTable`) is correct as originally documented.
...
1. **The value survives header → invoice on `CustInvoiceJour`, though the field name changes.** Verified directly against a real D365 dev-instance record rather than assumed from field documentation alone.

   **Why this needed checking:** `HotWaxAutoPostSettlementService` already proves `SalesId` survives the same header → invoice hop (it joins on `custInvoiceJour.SalesId` today), but that says nothing about whether the Customer Requisition Number value — a different field — is carried through the same way. D365's invoice-posting pipeline could in principle copy some header fields onto `CustInvoiceJour` and not others, and the two fields in question sit on different order types in our flow (`SalesId` is read off a return order's invoice; the settlement design needs this field off a **regular Sales Order's** invoice, since the exchange order is `OrderType = Sales order`, not `Returned order`) — so evidence from one doesn't automatically transfer to the other.
   ...
   **What was found:** Sales Order `001217` (`OrderType = Sales order`) had `CustomerRequisitionNumber = RMA 00085` set on its header. Its posted invoice's `CustInvoiceJour` record, found via `SysTableBrowser`, showed the same value under a column labeled "Purchase order" in that grid. Confirms the *value* is safe to join on for Step 3 of Section 6.4.2 — but the underlying field name shown by `SysTableBrowser` is the field's **label**, not its AOT name. The actual field turned out to be `PurchaseOrder` (displayName "Requisition"), not `PurchOrderFormNum` as initially assumed by analogy with `SalesTable` — only discovered once the X++ compiler rejected `PurchOrderFormNum` as invalid on `CustInvoiceJour`. See the note in Section 6.4.1.
