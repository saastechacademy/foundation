...
### Step 1: 
The staff gets the list of [Outstanding orders](https://github.com/hotwax/oms-documentation/blob/user-guides-pub/documents/integrate-with-hotwax/api/fulfillment/apis/outstanding-orders.md). 
...
* Triggers the process to get Shipping Label (doRateShopping) from the logistics company
  * If the facility is part of the `AUTO_SHIPPING_LABEL` facility group.
...
* Once we have Shipping label, Shipment can be moved to SHIPMENT_PACKED status.
  * If `isTrackingRequired` is set to **N** in the ProductStoreShipmentMeth entity for the given Carrier Shipment Method, then the user can pack the shipment without a shipping label or tracking code.

### On SHIPMENT_PACKED:
* Ship (Handover in case of BOPIS) the Shipment moves Shipment status to Shipped
* Fetch shipping label if auto shipping label is disabled at the facility and label is not already fetched.
